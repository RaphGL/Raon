...
static struct raon_token raon_lexer_lex_ident(struct raon_lexer *lex) {
  ...
  if (strcmp(tok.string_val, "true") == 0 ||
      strcmp(tok.string_val, "false") == 0) {
...
#define RAON_ONE_CHAR_TOKEN(literal, enum_type)                                \
  (struct raon_token) {                                                        \
    .type = enum_type, .char_val = literal, .from_x = lex->curr_x,             \
    .from_y = lex->curr_y, .to_x = lex->curr_x, .to_y = lex->curr_y,           \
  }
